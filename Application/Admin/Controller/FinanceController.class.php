...
class FinanceController extends AdminController
{
	...
	public function myzrQueren()
	{
		...
		$mo->execute('lock tables qq350166483_user_coin write,qq350166483_myzr write,qq350166483_finance write,qq350166483_invit write,qq350166483_user write');
		$rs = array();

		$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $myzr['userid']))->setInc($myzr['coinname'], $myzr['num']);
		$rs[] = $mo->table('qq350166483_myzr')->where(array('id' => $myzr['id']))->save(array('status' => 1, 'mum' => $myzr['num'], 'endtime' => time()));
	...
	public function myczQueren()
	{
		...
		$rs[] = $mo->table('qq350166483_mycz')->where(array('id' => $mycz['id']))->save(array('status' => 2, 'mum' => $mycz['num'], 'endtime' => time()));
	...
	public function qq350166483_financeExcel()
	{
		if (IS_POST) {
			$id = implode(',', $_POST['id']);
		}
	...
	public function qq350166483_financeAllExcel()
	{
	...
	public function myzcQueren($id = NULL)
	{
		...
		if ($zhannei) {
			$rs[] = $mo->table('qq350166483_myzr')->add(array('userid' => $zhannei['userid'], 'username' => $myzc['username'], 'coinname' => $coin, 'txid' => md5($myzc['username'] . $user_coin[$coin . 'b'] . time()), 'num' => $myzc['num'], 'fee' => $myzc['fee'], 'mum' => $myzc['mum'], 'addtime' => time(), 'status' => 1));
			$rs[] = $r = $mo->table('qq350166483_user_coin')->where(array('userid' => $zhannei['userid']))->setInc($coin, $myzc['mum']);
		}
		...
		if (0 < $myzc['fee']) {
			$rs[] = $mo->table('qq350166483_myzc_fee')->add(array('userid' => $fee_user['userid'], 'username' => $Coin['zc_user'], 'coinname' => $coin, 'num' => $myzc['num'], 'fee' => $myzc['fee'], 'mum' => $myzc['mum'], 'type' => 2, 'addtime' => time(), 'status' => 1));

			if ($mo->table('qq350166483_user_coin')->where(array($coin . 'b' => $Coin['zc_user']))->find()) {
				$rs[] = $mo->table('qq350166483_user_coin')->where(array($coin . 'b' => $Coin['zc_user']))->setInc($coin, $myzc['fee']);
				debug(array('lastsql' => $mo->table('qq350166483_user_coin')->getLastSql()), '新增费用');
			}
			else {
				$rs[] = $mo->table('qq350166483_user_coin')->add(array($coin . 'b' => $Coin['zc_user'], $coin => $myzc['fee']));
			}
		}
