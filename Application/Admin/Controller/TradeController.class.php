...
class TradeController extends AdminController
{
	public function index($field = NULL, $name = NULL, $market = NULL, $status = NULL,$type=0)
	{
		...
		$qq350166483_getSum = M('Trade')->where($where)->sum('mum');
		...
		$this->assign('qq350166483_count', $count);
		$this->assign('qq350166483_getSum', $qq350166483_getSum);
	...
	public function log($field = NULL, $name = NULL, $market = NULL,$type=NULL)
	{
		...
		$qq350166483_getSum = M('TradeLog')->where($where)->sum('mum');
		$Page = new \Think\Page($count, 15);
		$show = $Page->show();
	...
	public function market($field = NULL, $name = NULL)
	{
		...
		foreach($list as $k=>$v){
			if($v['begintrade']){
				$begintradeqq_350166483 = substr($v['begintrade'],0,5);
			}else{
				$begintradeqq_350166483 = "00:00";
			}
			if($v['endtrade']){
				$endtradeqq_350166483 = substr($v['endtrade'],0,5);
			}else{
				$endtradeqq_350166483 = "23:59";
			}
			
			
			$list[$k]['tradetimeqq350166483'] = $begintradeqq_350166483."-".$endtradeqq_350166483;
		}
	...
	public function marketEdit($id = NULL)
	{
		...
		if (empty($_POST)) {
			if (empty($id)) {
				...
				$market_qq350166483 = M('Market')->where(array('id' => $id))->find();
				$this->data = $market_qq350166483;

				if($market_qq350166483['begintrade']){
					$beginshi = explode(":",$market_qq350166483['begintrade'])[0];
					$beginfen = explode(":",$market_qq350166483['begintrade'])[1];
				}else{
					$beginshi = "00";
					$beginfen = "00";
				}
				
				if($market_qq350166483['endtrade']){
					$endshi = explode(":",$market_qq350166483['endtrade'])[0];
					$endfen = explode(":",$market_qq350166483['endtrade'])[1];
				}else{
					$endshi = "23";
					$endfen = "59";
				}
			...
			$this->assign('qq350166483_getCoreConfig',$qq350166483_getCoreConfig['qq350166483_indexcat']);
