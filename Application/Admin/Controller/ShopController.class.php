...
class ShopController extends AdminController
{
	...
	public function edit($id = NULL)
	{
		...
		if (APP_DEMO) {
			...
			$builder->keyText('qq350166483_awardcoinnum', '奖励币种数量', '整数,留空或者填写0 不奖励');
			$builder->keySelect('qq350166483_awardcoin', '奖励币种', '奖励币种', $coin_list);
	...
	public function chexiao($id = NULL)
	{
		...
		$mo->execute('lock tables qq350166483_user_coin write,qq350166483_shop_log write');
		$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $shoplog['userid']))->setInc($shoplog['coinname'], $shoplog['xuyao']);
		$rs[] = $mo->table('qq350166483_shop_log')->where(array('id' => $id))->save(array('status' => 2));
