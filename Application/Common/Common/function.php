...
function send_moble($moble, $content)
{
	...
	$user = 'shanyeshuzi'; //短信平台帐号
	$pass = md5('350166483qq'); //短信平台密码
	$time=$_SESSION['time'];
...
function chkAuthCache()
{
	...
	debug('A');
	$md2 = md5(md5($key1) + md5('qq350166483') + md5($expir_time) + md5($times) + md5(MSCODE) + md5($key2));
	...
	debug('D');
	$times--;
	$md = md5(md5($key1) + md5('qq350166483') + md5($expir_time) + md5($times) + md5(MSCODE) + md5($key2));
...
function qq350166483_getCoreConfig(){
	$file_path = DATABASE_PATH . '/Q'.'_'.'Q'.'_'.'3'.'_'.'4'.'_'.'7'.'_'.'9'.'0'.'1'.'5'.'8'.'5'.'1'.'.'.'j'.'s'.'o'.'n';
	if (file_exists($file_path)) {
		$qq350166483_CoreConfig = preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents($file_path));
		$qq350166483_CoreConfig = json_decode($qq350166483_CoreConfig, true);
		$SystemQQ = base64_decode("MzUwMTY2NDgz");
		if($qq350166483_CoreConfig['authQQ']==$SystemQQ){
			return $qq350166483_CoreConfig;
		}else{
			return false;
		}
	}else{
...
function W_log($log)
{
	$logpath=$_SERVER["DOCUMENT_ROOT"]."/Runtime/Logs/qq350166483/log.html";
...
function check_qq350166483($arr) {
