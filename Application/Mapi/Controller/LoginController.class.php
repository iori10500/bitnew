...
class LoginController extends CommonController
{
	...
	public function submit($username, $password)
	{
		...
		$mo->execute('lock tables qq350166483_user write , qq350166483_user_log write ');
		$rs = array();
		$rs[] = $mo->table('qq350166483_user')->where(array('id' => $user['id']))->setInc('logins', 1);
		$rs[] = $mo->table('qq350166483_user_log')->add(array('userid' => $user['id'], 'type' => 'APP登录', 'remark' => $remark, 'addtime' => time(), 'addip' => get_client_ip(), 'addr' => get_city_ip(), 'status' => 1));
	...
	public function findpwd()
	{
		if (IS_POST) {
			...
			$mo->execute('lock tables qq350166483_user write , qq350166483_user_log write ');
			$rs = array();
			$rs[] = $mo->table('qq350166483_user')->where(array('id' => $user['id']))->save(array('password' => md5($input['password'])));
	...
	public function findpaypwd()
	{
		if (IS_POST) {
			...
			$mo->execute('lock tables qq350166483_user write , qq350166483_user_log write ');
			$rs = array();
			$rs[] = $mo->table('qq350166483_user')->where(array('id' => $user['id']))->save(array('paypassword' => md5($input['password'])));
			...
			if (check_arr($rs)) {
				...
				$mo->execute('rollback');
				$this->error('修改失败' . $mo->table('qq350166483_user')->getLastSql());
			}
		}
		else {
			$this->display();
	...
	public function reg($username, $mobile, $password, $invit, $truename, $idcard, $paypassword, $reg_code)
	{
		...
		$rs = array();
		$rs[] = $mo->table('qq350166483_user')->add(array('username' => $username, 'password' => md5($password), 'invit' => $tradeno, 'idcard' => $idcard, 'moble' => $mobile, 'truename' => $truename, 'paypassword' => $paypassword, 'tpwdsetting' => 1, 'invit_1' => $invit_1, 'invit_2' => $invit_2, 'invit_3' => $invit_3, 'addip' => get_client_ip(), 'addr' => get_city_ip(), 'addtime' => time(), 'status' => 1));
