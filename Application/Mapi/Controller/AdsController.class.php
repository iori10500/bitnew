...
class AdsController extends CommonController {
    ...
    public function click($hash,$aid){
        ...
        $mo->execute('lock tables qq350166483_app_log write ,qq350166483_user_coin write');

        $rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $uid))->setDec($price_coin['name'], $Vip['price_num']);
        $rs[] = $mo->table('qq350166483_app_log')->add(
            array(
                'uid'     => $uid,
                'type'    => 'click_ads',
