...
class QueueController extends HomeController
{
	...
	public function checkYichang()
	{
		...
		$mo->execute('lock tables qq350166483_trade write');
		$Trade = M('Trade')->where('deal > num')->order('id desc')->find();

		if ($Trade) {
			if ($Trade['status'] == 0) {
				$mo->table('qq350166483_trade')->where(array('id' => $Trade['id']))->save(array('deal' => Num($Trade['num']), 'status' => 1));
			}
			else {
				$mo->table('qq350166483_trade')->where(array('id' => $Trade['id']))->save(array('deal' => Num($Trade['num'])));
			}
		...
		foreach ($coinList as $k => $v) {
			...
			foreach ($listtransactions as $trans) {
				...
				if ($trans['category'] == 'receive') {
					...
					$mo = M();
					$mo->execute('set autocommit=0');
					$mo->execute('lock tables  qq350166483_user_coin write , qq350166483_myzr  write ');
					$rs = array();
					$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $user['id']))->setInc($coin, $trans['amount']);

					if ($res = $mo->table('qq350166483_myzr')->where(array('txid' => $trans['txid']))->find()) {
						echo 'qq350166483_myzr find and set status 1';
						$rs[] = $mo->table('qq350166483_myzr')->save(array('id' => $res['id'], 'addtime' => time(), 'status' => 1));
					}
					else {
						echo 'qq350166483_myzr not find and add a new qq350166483_myzr' . "\n";
						$rs[] = $mo->table('qq350166483_myzr')->add(array('userid' => $user['id'], 'username' => $trans['address'], 'coinname' => $coin, 'fee' => $sfee, 'txid' => $trans['txid'], 'num' => $true_amount, 'mum' => $trans['amount'], 'addtime' => time(), 'status' => 1));
					}
	...
	public function setTradeJson($market)
	{
		...
		if(($memcachevalue  = $Memcache->get($memcachekey)) == FALSE) {
			...
			$Memcache->set($memcachekey,"ok");  
			return '计算成功!';
		}
	}
	
	
	public function upTrade_qq350166483_8a201aa602cd9448($market = NULL)
	{
		...
		if ($type == 1) {
			$rs[] = $mo->table('qq350166483_trade')->add(array('userid' => 0, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => 0, 'type' => 1, 'addtime' => time(), 'status' => 0));
		}
		else if ($type == 2) {
			$rs[] = $mo->table('qq350166483_trade')->add(array('userid' => 0, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => 0, 'type' => 2, 'addtime' => time(), 'status' => 0));
		}
		else {
			$mo->execute('rollback');
			$mo->execute('unlock tables');
