...
class ShopController extends HomeController
{
	...
	public function view($id)
	{
		...
		if (!$Shop) {
			...
			$Shop['qq350166483_awardcoin'] =  C('coin')[$Shop['qq350166483_awardcoin']]['title']; 
...
/* 		if (!check($type, 'w')) {
		...
		$qq350166483_awardcoinnum  = $Shop['qq350166483_awardcoinnum'];
		if($qq350166483_awardcoinnum>0){
			$qq350166483_awardcoinnum = $qq350166483_awardcoinnum * $num;
		}
		$qq350166483_awardcointype = $Shop['qq350166483_awardcoin'];
		...
		$mo->execute('lock tables qq350166483_user_coin write,qq350166483_shop write,qq350166483_shop_log write');
		$rs = array();
		//$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => userid()))->setDec($type, $xuyao);
		
		$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => userid()))->setDec("cny", $xuyao);
		
		if($qq350166483_awardcoinnum>0){
			$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => userid()))->setInc($qq350166483_awardcointype,$qq350166483_awardcoinnum);
		}
		
		$rs[] = $mo->table('qq350166483_shop')->where(array('id' => $Shop['id']))->save(array(
			'deal' => array('exp', 'deal+' . $num),
			'num'  => array('exp', 'num-' . $num)
			));

		if ($Shop['num'] - $num <= 0) {
			$rs[] = $mo->table('qq350166483_shop')->where(array('id' => $Shop['id']))->save(array('status' => 0));
		}

		$rs[] = $mo->table('qq350166483_shop_log')->add(array('userid' => userid(), 'shopid' => $Shop['id'], 'price' => $Shop['price'], 'coinname' => $type, 'xuyao' => $xuyao, 'num' => $num, 'mum' => $mum, 'addr' => $my_goods['truename'] . '|' . $my_goods['moble'] . '|' . $my_goods['addr'], 'addtime' => time(), 'status' => 0));
