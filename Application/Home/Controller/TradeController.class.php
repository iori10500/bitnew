...
class TradeController extends HomeController
{
	...
	public function upTrade($paypassword = NULL, $market = NULL, $price, $num, $type)
	{
		...
		$rs = array();
		$user_coin = $mo->table('qq350166483_user_coin')->where(array('userid' => userid()))->find();
	...
	public function matchingTrade($market = NULL)
	{
		...
		for (; true; ) {
			$buy = $mo->table('qq350166483_trade')->where(array('market' => $market,'userid' => array('gt',0), 'type' => 1, 'status' => 0))->order('price desc,id asc')->find();
			$sell = $mo->table('qq350166483_trade')->where(array('market' => $market,'userid' => array('gt',0), 'type' => 2, 'status' => 0))->order('price asc,id asc')->find();
	...
	public function matchingAutoTrade($market = NULL)
	{
		...
		for (; true; ) {
			$buy = $mo->table('qq350166483_trade')->where(array('market' => $market, 'type' => 1, 'userid'=>0,'status' => 0))->order('price desc,id asc')->find();
			$sell = $mo->table('qq350166483_trade')->where(array('market' => $market, 'type' => 2, 'userid'=>0,'status' => 0))->order('price asc,id asc')->find();
			...
			if ($buy && $sell && (0 <= floatval($buy['price']) - floatval($sell['price']))) {
				...
				$mo->execute('set autocommit=0');
				$mo->execute('lock tables qq350166483_trade write ,qq350166483_trade_log write ');
				
				$rs[] = $mo->table('qq350166483_trade')->where(array('id' => $buy['id']))->setInc('deal', $amount);
				$rs[] = $mo->table('qq350166483_trade')->where(array('id' => $sell['id']))->setInc('deal', $amount);
				
				$rs[] = $mo->table('qq350166483_trade_log')->add(array('userid' => 0, 'peerid' => 0, 'market' => $market, 'price' => $price, 'num' => $amount, 'mum' => $mum, 'type' => $type, 'fee_buy' => 0, 'fee_sell' => 0, 'addtime' => time(), 'status' => 1));
	...
	public function autoTrade()
	{
		...
		$type=rand(0,9)%2;
		$num=rand(1,100)/100;
		$lastprice = file_get_contents("http://vadio.cba123.cn/getlastprice.php");
		$lastprice=json_decode($lastprice,true);
		if($lastprice['note']){
		...
		if($type){
			$steprice=rand(-5000,0);
			$price=$price+$steprice;


		}else{
			$type=2;
			$steprice=rand(0,5000);
			$price=$price+$steprice;
		...
		$user = M('User')->where(array('id' => $autouserid))->find();
		...
		$user_coin = M('UserCoin')->where(array('userid' => $autouserid))->find();
		...
		if (C('coin')[$xnb]['fee_bili']) {
			if ($type == 2) {
				...
				if ($bili_user) {
					...
					if ($bili_keyi) {
						$bili_zheng = M()->query('select id,price,sum(num-deal)as nums from qq350166483_trade where userid=' . $autouserid . ' and status=0 and type=2 and market like \'%' . $xnb . '%\' ;');
		...
		$mo->execute('lock tables qq350166483_trade write ,qq350166483_user_coin write ,qq350166483_finance write');
		$rs = array();
		$user_coin = $mo->table('qq350166483_user_coin')->where(array('userid' => $autouserid))->find();
		...
		if ($type == 1) {
			...
			$finance = $mo->table('qq350166483_finance')->where(array('userid' => $autouserid))->order('id desc')->find();
			$finance_num_user_coin = $mo->table('qq350166483_user_coin')->where(array('userid' => $autouserid))->find();
			$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $autouserid))->setDec($rmb, $mum);
			$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $autouserid))->setInc($rmb . 'd', $mum);
			$rs[] = $finance_nameid = $mo->table('qq350166483_trade')->add(array('userid' => $autouserid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 1, 'addtime' => time(), 'status' => 0));
			$finance_mum_user_coin = $mo->table('qq350166483_user_coin')->where(array('userid' => $autouserid))->find();
			$finance_hash = md5($autouserid . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mum . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.btchanges.com');
			$finance_num = $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'];
			...
			$rs[] = $mo->table('qq350166483_finance')->add(array('userid' => $autouserid, 'coinname' => 'cny', 'num_a' => $finance_num_user_coin['cny'], 'num_b' => $finance_num_user_coin['cnyd'], 'num' => $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'], 'fee' => $mum, 'type' => 2, 'name' => 'trade', 'nameid' => $finance_nameid, 'remark' => '交易中心-委托买入-市场' . $market, 'mum_a' => $finance_mum_user_coin['cny'], 'mum_b' => $finance_mum_user_coin['cnyd'], 'mum' => $finance_mum_user_coin['cny'] + $finance_mum_user_coin['cnyd'], 'move' => $finance_hash, 'addtime' => time(), 'status' => $finance_status));
		}
		else if ($type == 2) {
			if ($user_coin[$xnb] < $num) {
				$this->error(C('coin')[$xnb]['title'] . '余额不足2！');
			...
			$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $autouserid))->setDec($xnb, $num);
			$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $autouserid))->setInc($xnb . 'd', $num);
			$rs[] = $mo->table('qq350166483_trade')->add(array('userid' => $autouserid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'type' => 2, 'addtime' => time(), 'status' => 0));
		}
		else {
			$mo->execute('rollback');
			$mo->execute('unlock tables');
