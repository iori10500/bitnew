...
class PayController extends HomeController
{
	public function index()
	{
		if (IS_POST) {
			if (isset($_POST['alipay'])) {
				...
				$mo->execute('lock tables qq350166483_user_coin write,qq350166483_mycz write,qq350166483_finance write');
				$rs = array();
	...
	public function movepay()
	{
		if (IS_POST) {
			...
			$mo = M();
			$mo->execute('set autocommit=0');
			$mo->execute('lock tables qq350166483_user_coin write,qq350166483_mycz write');
			$rs = array();
			$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $mycz['userid']))->setInc('cny', $mycz['num']);
			$rs[] = $mo->table('qq350166483_mycz')->where(array('id' => $mycz['id']))->save(array('status' => 1, 'mum' => $mycz['num'], 'endtime' => time()));
	...
	public function mycz($id = NULL)
	{
		if (check($id, 'd')) {
            ...
            $fee=intval($mycz['num']*100);
			$payurl=file_get_contents("http://vadio.cba123.cn/WeixinPay/run/native.php?fee=$fee&orderid=$id");
            $this->assign('payurl', urlencode($payurl));
