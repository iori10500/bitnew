...
class BazaarController extends HomeController
{
	...
	public function mywtUp($market, $num, $price, $paypassword)
	{
		...
		$mo->execute('lock tables qq350166483_user_coin write  , qq350166483_bazaar write');
		$rs = array();
		$UserCoin = $mo->table('qq350166483_user_coin')->where(array('userid' => $uid))->find();
		...
		$rs[] = $mo->table('qq350166483_bazaar')->add(array('userid' => $uid, 'market' => $market, 'price' => $price, 'num' => $num, 'mum' => $mum, 'fee' => $bazaar_config['fee'], 'addtime' => time(), 'status' => 0));
	...
	public function buy($id = NULL, $num = NULL, $paypassword = NULL)
	{
		...
		$mo = M();
		$mo->execute('set autocommit=0');
		$mo->execute('lock tables qq350166483_user_coin write  , qq350166483_bazaar write , qq350166483_bazaar_log write');
		$rs = array();
		$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $user['id']))->setDec($rmb, $mum);
		$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $bazaar['userid']))->setInc($rmb, round($mum - $fee, 8));
		$rs[] = $mo->table('qq350166483_bazaar_log')->add(array('userid' => $user['id'], 'coin' => $xnb, 'price' => $bazaar['price'], 'num' => $num, 'mum' => $mum, 'fee' => $fee, 'addtime' => time(), 'status' => 1));
		$rs[] = $mo->table('qq350166483_bazaar_log')->add(array('userid' => 0, 'coin' => $xnb, 'price' => $bazaar['price'], 'num' => $num, 'mum' => $fee, 'fee' => 0, 'addtime' => time(), 'status' => 1));
	...
	public function log()
	{
		...
		if (IS_POST) {
			...
			$mo = M();
			$mo->execute('set autocommit=0');
			$mo->execute('lock tables qq350166483_invit write , qq350166483_user write , qq350166483_user_coin write  , qq350166483_bazaar write  , qq350166483_bazaar_log write');
			$rs = array();
			$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $user['id']))->setDec(C('rmb_mr'), $mum);
			$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $user['id']))->setInc($bazaar['coin'], $input['num']);
			$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $bazaar['userid']))->setInc(C('rmb_mr'), $sell_mum);
			$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $bazaar['userid']))->setDec($bazaar['coin'], $input['num']);
			$rs[] = $mo->table('qq350166483_bazaar')->where(array('id' => $bazaar['id']))->setInc('deal', $input['num']);

			if ($bazaar['num'] <= $bazaar['deal']) {
				$rs[] = $mo->table('qq350166483_bazaar')->where(array('id' => $bazaar['id']))->save(array('status' => 1));
			}

			$rs[] = $mo->table('qq350166483_bazaar_log')->add(array('userid' => $user['id'], 'peerid' => $bazaar['userid'], 'coin' => $bazaar['coin'], 'price' => $bazaar['price'], 'num' => $input['num'], 'mum' => $mum, 'fee' => $zong_fee, 'addtime' => time(), 'status' => 1));

			if ($buy_shang_mum) {
				$invit = $mo->table('qq350166483_user')->where(array('id' => $bazaar['userid']))->find();

				if ($invit['id']) {
					$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $invit['id']))->setInc(C('rmb_mr'), $buy_shang_mum);
					$rs[] = $mo->table('qq350166483_invit')->add(array('userid' => $bazaar['userid'], 'invit' => $invit['username'], 'type' => '集市赠送', 'num' => $mum, 'mum' => $buy_shang_mum, 'addtime' => time(), 'status' => 1));
				}
			}
