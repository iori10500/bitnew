...
class ChartController extends HomeController
{
	public function getJsonData($market = NULL, $ajax = 'json')
	{
		if ($market) {
			...
			if(!$qq350166483_getCoreConfig){
				...
				$qq350166483_putong = $qq350166483_getCoreConfig['qq350166483_userTradeDetailNum'];
				$qq350166483_teshu = $qq350166483_getCoreConfig['qq350166483_specialUserTradeDetailNum'];
			}
			
			$limit = $qq350166483_putong;
			if(userid()){
				$usertype = M('User')->where(array($id => $userid))->getField('usertype');
				if($usertype ==1){
					$limit = $qq350166483_teshu;
				}else{
					$limit = $qq350166483_putong;
				}
			}
	...
	protected function getTradeBuy($market,$limit)
	{
		...
		$mo = M();
		$buy = $mo->query('select id,price,sum(num-deal)as nums from qq350166483_trade  where status=0 and type=1 and market =\'' . $market . '\' group by price order by price desc limit '.$limit);
		$data = '';
	...
	protected function getTradeSell($market,$limit)
	{
		$limit = intval($limit);
		$mo = M();
		$sell = $mo->query('select id,price,sum(num-deal)as nums from qq350166483_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit '.$limit);
		$data = '';
	...
	public function getMarketTrendJson()
	{
		...
		if(($memcachevalue  = $Memcache->get($memcachekey)) == FALSE) {
			...
			$Memcache->set($memcachekey,base64_encode(json_encode($json_data)));  
	...
	public function getMarketNewKqq350166483Json()
	{
		// TODO: SEPARATE
		$input = I('get.');
		$market = (is_array(C('market')[$input['market']]) ? trim($input['market']) : C('market_mr'));
