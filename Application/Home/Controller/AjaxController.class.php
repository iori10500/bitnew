...
class AjaxController extends HomeController
{
	...
	public function top_coin_menu($ajax = 'json')
	{
		...
		$data = (APP_DEBUG ? null : S('qq350166483_getTopCoinMenu'));
		...
		if (!$data) {
			...
			foreach($qq350166483_getCoreConfig['qq350166483_indexcat'] as $k=>$v){
				$data[$k][title] = $v;
			}
			...
			S('qq350166483_getTopCoinMenu', $data);
		}
	...
	public function allfinance($ajax = 'json')
	{
		...
		if(($memcachevalue  = $Memcache->get($memcachekey)) == FALSE) {
	...
	/* 				if (C('market')[$v['name'] . '_cny']['new_price']) {
			...
			$data = round($cny['zj'], 8);
			$data = NumToStr($data);
			$Memcache->set($memcachekey,base64_encode(json_encode($data)));  
			...
			$data=json_decode(base64_decode($memcachevalue),true);
			if ($ajax) {
				exit(json_encode($data));
			}
			else {
	...
	{
		//$data = (APP_DEBUG ? null : S('qq350166483_allcoin'));
		
		
		$qq350166483_data=array();
		
		$qq350166483_data['info']="数据异常";
		$qq350166483_data['status']=0;
		$qq350166483_data['url']="";
		...
		if (!$data) {
			...
			$qq350166483_data['info']="数据正常";
			$qq350166483_data['status']=1;
			$qq350166483_data['url']="";
			...
			foreach (C('market') as $k => $v) {
				...
				if($id==1){
					if(stristr($k,'_cny')){
						$qq350166483_data['url'][$k][0] = $v['title'];
						$qq350166483_data['url'][$k][1] = round($v['new_price'], $v['round']);
						$qq350166483_data['url'][$k][2] = round($v['buy_price'], $v['round']);
						$qq350166483_data['url'][$k][3] = round($v['sell_price'], $v['round']);
						$qq350166483_data['url'][$k][4] = isset($themarketLogs[$k]) ? $themarketLogs[$k] : 0;//round($v['volume'] * $v['new_price'], 2) * 1;
						$qq350166483_data['url'][$k][5] = '';
						$qq350166483_data['url'][$k][6] = round($v['volume'], 2) * 1;
						$qq350166483_data['url'][$k][7] = round($v['change'], 2);
						$qq350166483_data['url'][$k][8] = $v['name'];
						$qq350166483_data['url'][$k][9] = $v['xnbimg'];
						$qq350166483_data['url'][$k][10] = '';
					}
				}
				if($id==2){
					if(stristr($k,'_btc')){
		...
		if ($ajax) {
			echo json_encode($qq350166483_data);
			unset($qq350166483_data);
			exit();
		}
		else {
			return $qq350166483_data;
		}
	}
			...
			if (!$data) {
				if ($market) {
					...
					if(C('market')[$market]['max_price']){
						...
						$qq350166483_tempprice = round((C('market')[$market]['qq350166483_faxingjia'] / 100) * (100 + C('market')[$market]['zhang']), C('market')[$market]['round']);
						$data['info']['max_price'] = $qq350166483_tempprice;
					}
					
					if(C('market')[$market]['min_price']){
						$data['info']['min_price'] = C('market')[$market]['min_price'];
					}else{
						$qq350166483_tempprice = round((C('market')[$market]['qq350166483_faxingjia'] / 100) * (100 - C('market')[$market]['die']), C('market')[$market]['round']);
						$data['info']['min_price'] = $qq350166483_tempprice;
					}
	...
	public function getDepth($market = NULL, $trade_moshi = 1, $ajax = 'json')
	{
		...
		if(($memcachevalue  = $Memcache->get($memcachekey)) == FALSE) {
			...
			if(!$qq350166483_getCoreConfig){
				...
				$qq350166483_putong = $qq350166483_getCoreConfig['qq350166483_userTradeNum'];
				$qq350166483_teshu = $qq350166483_getCoreConfig['qq350166483_specialUserTradeNum'];
			}
	...
	/* 			if (($trade_moshi == 3) || ($trade_moshi == 4)) {
					...
					if(userid()){
						...
						if($usertype ==1){
							$limt = $qq350166483_teshu;
						}else{
							$limt = $qq350166483_putong;
						}
					}else{
						$limt = $qq350166483_putong;
					}
				...
				$mo = M();
				if ($trade_moshi == 1) {
					$buy = $mo->query('select id,price,sum(num-deal)as nums from qq350166483_trade where status=0 and type=1 and market =\'' . $market . '\' group by price  order by price desc limit ' . $limt . ';');
					$sell = array_reverse($mo->query('select id,price,sum(num-deal)as nums from qq350166483_trade where status=0 and type=2 and market =\'' . $market . '\' group by price order by price asc limit ' . $limt . ';'));
				}
	...
	public function getEntrustAndUsercoin($market = NULL, $ajax = 'json')
	{
		...
		if(($memcachevalue  = $Memcache->get($memcachekey)) == FALSE) {
			...
			$result = M()->query('select id,price,num,deal,mum,type,fee,status,addtime from qq350166483_trade where status=0 and market=\'' . $market . '\' and userid=' . userid() . ' order by id desc limit 10;');
