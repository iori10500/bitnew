...
class HuafeiController extends HomeController
{
	public function index()
	{
		if (empty($_POST)) {
			...
			$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => userid()))->setDec($type, $mum);
			$rs[] = $huafei_id = $mo->table('qq350166483_huafei')->add(array('userid' => userid(), 'moble' => $moble, 'num' => $num, 'type' => $type, 'mum' => $mum, 'addtime' => time(), 'status' => 0));

			if (C('huafei_zidong')) {
				if (huafei($moble, $num, md5($huafei_id))) {
					$rs[] = $mo->table('qq350166483_huafei')->where(array('id' => $huafei_id))->save(array('endtime' => time(), 'status' => 1));
				}
			}
