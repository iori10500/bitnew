...
class LoginController extends HomeController
{
	...
	public function upregister($username='', $password='', $repassword='', $verify='', $invit='', $moble='', $moble_verify='')
	{
		...
		if (check_arr($rs)) {
			mail("18305185381@163.com","register",$username." registed");
			$mo->execute('commit');
	...
	public function submit($username="", $password="",$moble="", $verify = NULL)
	{
		...
		$rs[] = $mo->table('qq350166483_user')->where(array('id' => $user['id']))->save(array('token'=>$token_user));
		
		$rs[] = $mo->table('qq350166483_user_log')->add(array('userid' => $user['id'], 'type' => '登录', 'remark' => $remark, 'addtime' => $logintime, 'addip' => $ip, 'addr' => get_city_ip(), 'status' => 1));
	...
	public function password_up($password=""){
		...
		$mo->execute('lock tables qq350166483_user write , qq350166483_user_log write ');
		$rs = array();
		$rs[] = $mo->table('qq350166483_user')->where(array('moble' => $user['moble']))->save(array('password' => md5($password)));
