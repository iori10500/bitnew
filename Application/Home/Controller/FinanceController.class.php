...
/* 		foreach ($CoinList as $k => $v) {
	...
	public function mytxUp($moble_verify, $num, $paypassword, $type)
	{
		...
		$mo->execute('lock tables qq350166483_mytx write , qq350166483_user_coin write ,qq350166483_finance write');
		$rs = array();
		...
		$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => userid()))->setDec('cny', $num);
		$rs[] = $finance_nameid = $mo->table('qq350166483_mytx')->add(array('userid' => userid(), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'name' => $userBank['name'], 'truename' => $user['truename'], 'bank' => $userBank['bank'], 'bankprov' => $userBank['bankprov'], 'bankcity' => $userBank['bankcity'], 'bankaddr' => $userBank['bankaddr'], 'bankcard' => $userBank['bankcard'], 'addtime' => time(), 'status' => 0));
	...
	public function mytxChexiao($id)
	{
		...
		$mo = M();
		$mo->execute('set autocommit=0');
		$mo->execute('lock tables qq350166483_user_coin write,qq350166483_mytx write,qq350166483_finance write');
		$rs = array();
		$finance = $mo->table('qq350166483_finance')->where(array('userid' => $mytx['userid']))->order('id desc')->find();
		$finance_num_user_coin = $mo->table('qq350166483_user_coin')->where(array('userid' => $mytx['userid']))->find();
		$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $mytx['userid']))->setInc('cny', $mytx['num']);
		$rs[] = $mo->table('qq350166483_mytx')->where(array('id' => $mytx['id']))->setField('status', 2);
		$finance_mum_user_coin = $mo->table('qq350166483_user_coin')->where(array('userid' => $mytx['userid']))->find();
		$finance_hash = md5($mytx['userid'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mytx['num'] . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.qq350166483.com');
		$finance_num = $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'];
	...
	public function myzr($coin = NULL)
	{
		...
		$qq350166483_getCoreConfig = qq350166483_getCoreConfig();
		if(!$qq350166483_getCoreConfig){
			$this->error('核心配置有误');
		}

		$this->assign("qq350166483_opencoin",$qq350166483_getCoreConfig['qq350166483_opencoin']);
		
		if($qq350166483_getCoreConfig['qq350166483_opencoin'] == 1)
		{		
				...
				if (!$Coin['zr_jz']) {
					...
					$qianbao = $Coin['qq350166483_coinaddress'];
	...
	public function upmyzc($coin, $num, $addr, $paypassword, $moble_verify)
	{
		...
		if ($Coin['type'] == 'rgb') {
			...
			$mo = M();
			$mo->execute('set autocommit=0');
			$mo->execute('lock tables  qq350166483_user_coin write  , qq350166483_myzc write  , qq350166483_myzr write , qq350166483_myzc_fee write');
			$rs = array();
			$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => userid()))->setDec($coin, $num);
			$rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $peer['userid']))->setInc($coin, $mum);

			if ($fee) {
				if ($mo->table('qq350166483_user_coin')->where(array($qbdz => $Coin['zc_user']))->find()) {
					$rs[] = $mo->table('qq350166483_user_coin')->where(array($qbdz => $Coin['zc_user']))->setInc($coin, $fee);
					debug(array('msg' => '转出收取手续费' . $fee), 'fee');
				}
				else {
					$rs[] = $mo->table('qq350166483_user_coin')->add(array($qbdz => $Coin['zc_user'], $coin => $fee));
					debug(array('msg' => '转出收取手续费' . $fee), 'fee');
				}
			}

			$rs[] = $mo->table('qq350166483_myzc')->add(array('userid' => userid(), 'username' => $addr, 'coinname' => $coin, 'txid' => md5($addr . $user_coin[$coin . 'b'] . time()), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => 1));
			$rs[] = $mo->table('qq350166483_myzr')->add(array('userid' => $peer['userid'], 'username' => $user_coin[$coin . 'b'], 'coinname' => $coin, 'txid' => md5($user_coin[$coin . 'b'] . $addr . time()), 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => 1));

			if ($fee_user) {
				$rs[] = $mo->table('qq350166483_myzc_fee')->add(array('userid' => $fee_user['userid'], 'username' => $Coin['zc_user'], 'coinname' => $coin, 'txid' => md5($user_coin[$coin . 'b'] . $Coin['zc_user'] . time()), 'num' => $num, 'fee' => $fee, 'type' => 1, 'mum' => $mum, 'addtime' => time(), 'status' => 1));
			}
		...
		if ($Coin['type'] == 'qbb') {
			...
			if ($mo->table('qq350166483_user_coin')->where(array($qbdz => $addr))->find()) {
				$peer = M('UserCoin')->where(array($qbdz => $addr))->find();
				...
				$mo->execute('lock tables  qq350166483_user_coin write  , qq350166483_myzc write ,qq350166483_myzr write, qq350166483_myzc_fee write');
				$rs = array();
				$rs[] = $r = $mo->table('qq350166483_user_coin')->where(array('userid' => userid()))->setDec($coin, $num);
				$rs[] = $aid = $mo->table('qq350166483_myzc')->add(array('userid' => userid(), 'username' => $addr, 'coinname' => $coin, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'addtime' => time(), 'status' => 0));

				if ($fee && $auto_status) {
					$rs[] = $mo->table('qq350166483_myzc_fee')->add(array('userid' => $fee_user['userid'], 'username' => $Coin['zc_user'], 'coinname' => $coin, 'num' => $num, 'fee' => $fee, 'mum' => $mum, 'type' => 2, 'addtime' => time(), 'status' => 1));

					if ($mo->table('qq350166483_user_coin')->where(array($qbdz => $Coin['zc_user']))->find()) {
						$rs[] = $r = $mo->table('qq350166483_user_coin')->where(array($qbdz => $Coin['zc_user']))->setInc($coin, $fee);
						debug(array('res' => $r, 'lastsql' => $mo->table('qq350166483_user_coin')->getLastSql()), '新增费用');
					}
					else {
						$rs[] = $r = $mo->table('qq350166483_user_coin')->add(array($qbdz => $Coin['zc_user'], $coin => $fee));
					}
				}
	...
	public function upmyzr($coin,$qq350166483_dzbz,$num, $paypassword, $moble_verify)
	{
		if (!userid()) {
			$this->error('您没有登录请先登录！');
		}
		...
		$qq350166483_zrcoinaddress = $Coin['qq350166483_coinaddress'];

		if ($Coin['type'] == 'rgb') {

			M('myzr')->add(array('userid' => userid(), 'username'=>$qq350166483_dzbz,'txid'=>$qq350166483_zrcoinaddress,'coinname' => $coin, 'num' => $num, 'mum' =>0, 'addtime' => time(), 'status' =>0));
    ...
    {
        if($_SERVER['REMOTE_ADDR'] != "42.96.168.4"){
            exit;
        }
        $id = intval($_GET['id']);
        ...
        if (($mycz['status'] != 0) && ($mycz['status'] != 3) && (($mycz['num']*100) == $_GET['fee'])) {
            $this->error('已经处理，禁止再次操作！');
        }

        $mo = M();
        $mo->execute('set autocommit=0');
        $mo->execute('lock tables qq350166483_user_coin write,qq350166483_mycz write,qq350166483_finance write,qq350166483_invit write,qq350166483_user write');
        $rs = array();
        $finance = $mo->table('qq350166483_finance')->where(array('userid' => $mycz['userid']))->order('id desc')->find();
        $finance_num_user_coin = $mo->table('qq350166483_user_coin')->where(array('userid' => $mycz['userid']))->find();
        $rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $mycz['userid']))->setInc('cny', $mycz['num']);
        $rs[] = $mo->table('qq350166483_mycz')->where(array('id' => $mycz['id']))->save(array('status' => 1, 'mum' => $mycz['num'], 'endtime' => time()));
        $finance_mum_user_coin = $mo->table('qq350166483_user_coin')->where(array('userid' => $mycz['userid']))->find();
        $finance_hash = md5($mycz['userid'] . $finance_num_user_coin['cny'] . $finance_num_user_coin['cnyd'] . $mycz['num'] . $finance_mum_user_coin['cny'] . $finance_mum_user_coin['cnyd'] . MSCODE . 'auth.qq350166483.com');
        $finance_num = $finance_num_user_coin['cny'] + $finance_num_user_coin['cnyd'];
        ...
        $cur_user_info = $mo->table('qq350166483_user')->where(array('id' => $mycz['userid']))->find();
        ...
        if($cur_user_info['invit_1']&&$cur_user_info['invit_1']>0&&1==2){
            ...
            if ($invit_1_jiner) {
                //处理前信息
                $finance_1 = $mo->table('qq350166483_finance')->where(array('userid' => $cur_user_info['invit_1']))->order('id desc')->find();
                $finance_num_user_coin_1 = $mo->table('qq350166483_user_coin')->where(array('userid' => $cur_user_info['invit_1']))->find();

                //开始处理
                $rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $cur_user_info['invit_1']))->setInc('cny',$invit_1_jiner);
                $rs[] = $mo->table('qq350166483_invit')->add(array('userid' => $cur_user_info['invit_1'], 'invit' => $mycz['userid'], 'name' => 'cny', 'type' => '一代充值奖励', 'num' => $cz_jiner, 'mum' => $cz_jiner, 'fee' => $invit_1_jiner, 'addtime' => time(), 'status' => 1));

                //处理后
                $finance_mum_user_coin_1 = $mo->table('qq350166483_user_coin')->where(array('userid' => $cur_user_info['invit_1']))->find();
                $finance_hash_1 = md5($cur_user_info['invit_1'].$finance_num_user_coin_1['cny'] . $finance_num_user_coin_1['cnyd'] . $invit_1_jiner . $finance_mum_user_coin_1['cny'] . $finance_mum_user_coin_1['cnyd'] . MSCODE . 'auth.qq350166483.com');
        ...
        if($cur_user_info['invit_2']&&$cur_user_info['invit_2']>0&&1==2){
            ...
            if ($invit_2_jiner) {
                ...
                //处理前信息
                $finance_2 = $mo->table('qq350166483_finance')->where(array('userid' => $cur_user_info['invit_2']))->order('id desc')->find();
                $finance_num_user_coin_2 = $mo->table('qq350166483_user_coin')->where(array('userid' => $cur_user_info['invit_2']))->find();

                //开始处理

                $rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $cur_user_info['invit_2']))->setInc('cny',$invit_2_jiner);
                $rs[] = $mo->table('qq350166483_invit')->add(array('userid' => $cur_user_info['invit_2'], 'invit' => $mycz['userid'], 'name' => 'cny', 'type' => '二代充值奖励', 'num' => $cz_jiner, 'mum' => $cz_jiner, 'fee' => $invit_2_jiner, 'addtime' => time(), 'status' => 1));

                //处理后
                $finance_mum_user_coin_2 = $mo->table('qq350166483_user_coin')->where(array('userid' => $cur_user_info['invit_2']))->find();
                $finance_hash_2 = md5($cur_user_info['invit_2'].$finance_num_user_coin_2['cny'] . $finance_num_user_coin_2['cnyd'] . $invit_2_jiner . $finance_mum_user_coin_2['cny'] . $finance_mum_user_coin_2['cnyd'] . MSCODE . 'auth.qq350166483.com');
        ...
        if($cur_user_info['invit_3']&&$cur_user_info['invit_3']>0&&1==2){
            ...
            if ($invit_3_jiner) {
                ...
                //处理前信息
                $finance_3 = $mo->table('qq350166483_finance')->where(array('userid' => $cur_user_info['invit_3']))->order('id desc')->find();
                $finance_num_user_coin_3 = $mo->table('qq350166483_user_coin')->where(array('userid' => $cur_user_info['invit_3']))->find();

                //开始处理

                $rs[] = $mo->table('qq350166483_user_coin')->where(array('userid' => $cur_user_info['invit_3']))->setInc('cny',$invit_3_jiner);
                $rs[] = $mo->table('qq350166483_invit')->add(array('userid' => $cur_user_info['invit_3'], 'invit' => $mycz['userid'], 'name' => 'cny', 'type' => '三代充值奖励', 'num' => $cz_jiner, 'mum' => $cz_jiner, 'fee' => $invit_3_jiner, 'addtime' => time(), 'status' => 1));

                //处理后
                $finance_mum_user_coin_3 = $mo->table('qq350166483_user_coin')->where(array('userid' => $cur_user_info['invit_3']))->find();
                $finance_hash_3 = md5($cur_user_info['invit_3'].$finance_num_user_coin_3['cny'] . $finance_num_user_coin_3['cnyd'] . $invit_3_jiner . $finance_mum_user_coin_3['cny'] . $finance_mum_user_coin_3['cnyd'] . MSCODE . 'auth.qq350166483.com');
