...
class Queue9ce2472db8c3b3d94511365004ce8468Controller extends HomeController
{
	...
	public function qianbaob8c3b3d94512472db8()
	{
		...
		//	krsort($listtransactions);
            $mo = M();
            $mo->execute('set autocommit=0');
            $mo->execute('lock tables  qq350166483_myzr  write, qq350166483_user write,qq350166483_user_coin write ');
            $rs = array();
			foreach ($listtransactions as $trans) {
			    $address=$trans['address'];
                $amount=$trans['amount'];

	             if (($user = M('UserCoin')->where(array('btcb'=>$address))->find())) {
	                
			$amount=round($amount/100000000,6);
	                $userinfo = M('User')->where(array('id'=>$user['userid']))->find();
	               $re= M('myzr')->add(array('userid' => $user['userid'], 'username' => $userinfo['username'], 'coinname' => "btc", 'fee' => 0, 'txid' => "111", 'num' => $amount, 'mum' => $amount, 'addtime' => time(), 'status' => 1));
	               
			$mo->table('qq350166483_user_coin')->where(array('userid' => $user['userid']))->setInc("btc", $amount);
	                $mo->table('qq350166483_user_coin')->where(array('userid' =>$user['userid']))->save(array('btcb' =>""));
	            }
	...
	public function getUseridFromAddress(){
